...
# 计算预测关节位置与真实关节位置之间的欧式距离
def compute_jpe(S1, S2):# S1和S2分别为预测和真实的关节位置
...
#使用全局对齐来调整预测关节位置，使其与真实关节位置对齐
#通过对整个关节序列进行旋转、缩放和平移，使预测结果与真实值最匹配
...
def compute_foot_distance(target_output,
    ...
    distance = (pred_feet_loc.unsqueeze(1) - gt_points.unsqueeze(0).to(pred_feet_loc.device)).norm(2, dim=-1)
    distance = torch.min(distance, dim=1)[0]

    return distance.cpu().numpy()

# 脚滑指标，在脚着地时刻计算预测模型中脚的移动距离（大幅移动说明滑动）
...
# 计算位移误差
# 先进行全局对齐，然后计算根节点的偏移距离，再根据总位移进行归一化处理
